...
func exchangeConnInfoProto(ctx context.Context, sender *io.PipeWriter, chRecvOrErr <-chan readerOrError, sinfo *selfInfo, cancelReq context.CancelFunc) (*connInfo, error) {
	infoEnc, _ := json.Marshal(sinfo)
	err := sendPacket(sender, infoEnc)
	if err != nil {
		return nil, fmt.Errorf("[155]failed to communicate with the bridge: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[180]failed to communicate with the bridge: %w", err)
	}
